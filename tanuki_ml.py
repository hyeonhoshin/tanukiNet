...
def generate_model(input_shape, pool_size):
    ...
    conv1 = Conv2D(60, 3, activation = 'relu', padding = 'valid')(batch)
    conv1 = Conv2D(50, 3, activation = 'relu', padding = 'valid')(conv1)
    pool1 = MaxPooling2D(pool_size=pool_size)(conv1)

    batch = BatchNormalization()(pool1)
    conv2 = Conv2D(40, 3, activation = 'relu', padding = 'valid')(batch)
    drop1 = Dropout(0.2)(conv2)
    conv2 = Conv2D(30, 3, activation = 'relu', padding = 'valid')(drop1)
    drop2 = Dropout(0.2)(conv2)
    conv2 = Conv2D(20, 3, activation = 'relu', padding = 'valid')(drop2)
    drop3 = Dropout(0.2)(conv2)
    pool2 = MaxPooling2D(pool_size=pool_size)(drop3)
    
    batch = BatchNormalization()(pool2)
    conv3 = Conv2D(10, 3, activation = 'relu', padding = 'valid')(batch)
    drop4 = Dropout(0.2)(conv3)
    conv3 = Conv2D(5, 3, activation = 'relu', padding = 'valid')(drop4)
